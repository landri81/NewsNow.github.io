...
critical=crise majeure, high=décision importante, medium=notable, low=tendance. UNIQUEMENT LE JSON.`
    ...
    if (response.status === 429) {
      if (cachedNews) return res.json({ articles: cachedNews, cached: true, fetchedAt: lastFetch });
      return res.status(503).json({ error: "Trop de requêtes. Réessayez dans 1 minute." });
    }
